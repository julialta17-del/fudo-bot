...
# --- 1. CONFIGURACIÓN ---
ruta_excel = r'C:\fudo_bot\descargas\temp_excel\ventas.xls'
...
def ejecutar_sistema_completo():
    ...
    # --- HISTORICO ---
    print("3. Actualizando Histórico...")
    try:
        sheet_hist = spreadsheet.worksheet("Historico")
    except:
        sheet_hist = spreadsheet.add_worksheet(title="Historico", rows="20000", cols="30")
        sheet_hist.append_row(df_v.columns.tolist())

    ids_viejos = [str(val).split('.')[0] for val in sheet_hist.col_values(1)]
    df_v['Id_Str'] = df_v['Id'].astype(str).str.replace('.0', '', regex=False).str.strip()
    nuevos = df_v[~df_v['Id_Str'].isin(ids_viejos)].copy()

    if not nuevos.empty:
        nuevos_para_subir = nuevos.drop(columns=['Id_Str']).replace([np.nan, np.inf, -np.inf], None).fillna("")
        datos_nuevos = [[str(item) if (pd.notnull(item) and item != "nan") else "" for item in fila] for fila in nuevos_para_subir.values.tolist()]
        sheet_hist.append_rows(datos_nuevos)
        print(f"✅ Sincronizadas {len(nuevos)} filas nuevas.")
    ...
    col_pago = 'Medio de Pago' # Ajustado a tu nombre real
